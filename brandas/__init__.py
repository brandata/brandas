    ...
    def _check_input_types(self, data):
        ...
        for k, v in data.items():
            if not isinstance(k, str):
                raise TypeError('keys in the `data` dict must be a string')
            if not isinstance(v, np.ndarray):
                raise TypeError('values in the `data` dict must be numpy arrays')
            if v.ndim != 1:
                raise ValueError('values of `data` must be a 1 dimensional array')
